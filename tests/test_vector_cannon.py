...
from right_arm.vector_cannon.gateway import CreditSnapshot, PROVIDER_SPECS, get_provider_spec
from right_arm.vector_cannon.issue_command import parse_issue_body
from right_arm.vector_cannon.repo_tools import RepositoryReader
from right_arm.vector_cannon.runner import CannonRunResult, VectorCannon
...
class _TruncatingGateway:
    ...
    def find_model(self, model: str):
        return {"id": model, "pricing": {"input": "0.000001", "output": "0.000001"}}

    def catalog_pricing(self, model):
        return Decimal("0.000001"), Decimal("0.000001")

    def declared_free_model(self, model: str) -> bool:
        return False

    def credits(self) -> CreditSnapshot:
        return CreditSnapshot(balance=Decimal("10.00"), total_used=Decimal("0"))
