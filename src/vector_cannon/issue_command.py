...
PROMPTS = {"repo_recon": "prompts/vector_cannon/repo_recon.md"}
...
def parse_issue_body(body: str, *, allowed_owner: str) -> IssueShot:
    values: dict[str, str] = {}
    allowed_fields = {"provider", "model", "target_repo", "target_ref", "prompt", "reasoning", "max_usd", "max_steps", "max_output_tokens", "input_price_per_million", "output_price_per_million", "tag"}
    ...
    missing = sorted({"model", "target_repo"} - values.keys())
    if missing:
    ...
    if provider not in PROVIDER_SPECS: raise ValueError("unsupported provider")
    if not SAFE_MODEL.fullmatch(model) or ".." in model: raise ValueError("invalid model id")
    if not SAFE_REPO.fullmatch(target_repo): raise ValueError("invalid target_repo")
    if target_repo.split("/", 1)[0].casefold() != allowed_owner.casefold(): raise ValueError("target_repo owner is not allowed")
    if not SAFE_REF.fullmatch(target_ref) or ".." in target_ref: raise ValueError("invalid target_ref")
    if prompt not in PROMPTS: raise ValueError("unsupported prompt")
    if reasoning is not None and not SAFE_REASONING.fullmatch(reasoning): raise ValueError("invalid reasoning value")
    if not SAFE_TAG.fullmatch(tag): raise ValueError("invalid tag")
    ...
    if not (Decimal("0") < max_usd <= Decimal("50")): raise ValueError("max_usd must be >0 and <=50")
    if not (1 <= max_steps <= 40): raise ValueError("max_steps must be 1..40")
    if not (1 <= max_output_tokens <= 50000): raise ValueError("max_output_tokens must be 1..50000")
    ...
    return IssueShot(provider, model, target_repo, target_ref, PROMPTS[prompt], reasoning, max_usd, max_steps, max_output_tokens, input_price, output_price, tag)
