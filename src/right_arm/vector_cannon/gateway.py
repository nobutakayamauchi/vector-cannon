...
PROVIDER_SPECS: dict[str, ProviderSpec] = {
    "vercel": ProviderSpec(
        ...
        model_catalog_auth=False,
        credits_path="/credits",
        catalog_pricing_per_token=True,
        reasoning_style="reasoning_object",
    ),
    "openai": ProviderSpec(
        provider_id="openai",
    ...
    def _request(
        ...
        *,
        ...
        try:
            ...
            raise GatewayError(
                f"{self.provider_id} HTTP {exc.code}: {body[:1000]}"
            ) from exc
    ...
    def add_request_metadata(self, request: dict[str, Any], *, tag: str) -> None:
        if self.provider_id == "vercel":
            request["providerOptions"] = {
                "gateway": {"tags": ["vector-cannon", tag]}
            }
...
class VercelGateway(OpenAICompatibleGateway):
    """Compatibility wrapper for the original single-provider Vector Cannon API."""
